#include "pch.h"
balasalu::balasalu(int _x, int _y, int _dx, int _dy) {
	Bitmap^ img = gcnew Bitmap("balasalu.png");
	x = _x; y = _y;
	dy = _dy;
	dx = _dx;
	ancho = img->Width;
...
balasalu::~balasalu() {}
void balasalu::dibujar(Graphics^ g) {
	Bitmap^ imgtranspa = gcnew Bitmap("balasalu.png");
...
void balasalu::mover(Graphics^ g) {
	x = x + dx;
	y = y + dy;
	if (x + dx < 0)
		x = 0;
